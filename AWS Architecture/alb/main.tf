##ALB의 보안그룹 먼저 이건 ALB여서 따로 생성해서 적용
# 보안 그룹
# 위치 : EC2 > 네트워크 및 보안 > 보안 그룹
# https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/security_group
resource "aws_security_group" "hansom_alb_sg" {
  name        = "hansom-alb-sg-${var.service_type}"
  description = "hansom alb security group production"
  vpc_id      = var.vpc_id
...
resource "aws_lb_target_group" "tg" {
  name        = "hansom-tg-${var.service_type}" //대상 그룹 이름
  port        = 3000                            //포트
  protocol    = "HTTP"                          //프로토콜
  ...
  tags = {
    Name    = "hansom-tg-${var.service_type}"
    Service = "hansom-${var.service_type}"
  }
}
