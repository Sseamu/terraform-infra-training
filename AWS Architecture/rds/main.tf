...
resource "aws_security_group" "sg" {
  name        = "hansom-db-sg-${var.service_type}" //보안그룹이름을 서비스타입으로 구분하기위해서
  description = "hansom db security group production"
  vpc_id      = var.vpc_id
  # 인바운드 규칙   
  ingress {
    description = ""
  ...
  tags = {
    Name    = "hansom-db-sg-${var.service_type}"
...
resource "aws_db_instance" "rds" {
  identifier             = "hansom-db-${var.service_type}"     //DB 인스턴스 식별자
  ...
  tags = {
    Name    = "hansom-db-${var.service_type}"
    Service = "hansom-${var.service_type}"
  }
}
